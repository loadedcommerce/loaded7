      ...
      <?php
        if (defined('DISABLE_ADD_TO_CART') && DISABLE_ADD_TO_CART == 1 && ($lC_Template->getModule() == 'products' || $lC_Template->getModule() == 'reviews') ) {
          ...
          if (file_exists('templates/' . $lC_Template->getCode() . '/classes/output.php')) {
            ...
            ?>
            $(":contains('<?php echo addslashes($lC_Language->get('button_buy_now')); ?>')").closest('button').removeClass("btn-success").addClass("btn-default").addClass("disabled").html('<?php echo $lC_Language->get('out_of_stock'); ?>');
