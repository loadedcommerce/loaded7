...
*/
  ...
  */
  if (!function_exists('lc_href_link')) {
    function lc_href_link($page = null, $parameters = null, $connection = 'NONSSL', $add_session_id = true, $search_engine_safe = true, $use_full_address = false) {
      ...
      if ($connection == 'AUTO') {
        if ( ($request_type == 'https') && (ENABLE_SSL === true) ) {
  ...
  if (!function_exists('get_permalink_query')) {
    function get_permalink_query($key) {
      ...
      if ($Qpermalink->numberOfRows() > 0) {
        ...
        $Qpid->bindValue(':products_keyword', $key);
        $Qpid->bindInt(':language_id', $lC_Language->getID());
        $Qpid->execute();
        
        return get_parents_path($Qpid->valueInt('categories_id'));        
      }
    }
  }
  ...
  * @return array
  */
  if (!function_exists('get_parents_path')) {
    function get_parents_path($id = null) {
      global $lC_Database;
