...
/**
  ...
  @version    $Id: vantiv.php v1.0 2013-08-08 datazen $
*/
include_once(DIR_FS_CATALOG . 'includes/classes/transport.php');

class lC_Payment_vantiv extends lC_Payment {     
 /**
  ...
  * Constructor
  */      
  public function lC_Payment_vantiv() {
    global $lC_Language;

    $this->_title = $lC_Language->get('payment_vantiv_title');
    $this->_method_title = $lC_Language->get('payment_vantiv_method_title');
    $this->_status = (defined('ADDONS_PAYMENT_VANTIV_PAYMENTS_STATUS') && (ADDONS_PAYMENT_VANTIV_PAYMENTS_STATUS == '1') ? true : false);
    $this->_sort_order = (defined('ADDONS_PAYMENT_VANTIV_PAYMENTS_SORT_ORDER') ? ADDONS_PAYMENT_VANTIV_PAYMENTS_SORT_ORDER : null);

    if (defined('ADDONS_PAYMENT_VANTIV_PAYMENTS_STATUS')) {
      $this->initialize();
    }
  }
 /**
  ...
  */
  public function initialize() {
    ...
    if ((int)ADDONS_PAYMENT_VANTIV_PAYMENTS_ORDER_STATUS_ID > 0) {
      $this->order_status = ADDONS_PAYMENT_VANTIV_PAYMENTS_ORDER_STATUS_ID;
    }
    
    if ((int)ADDONS_PAYMENT_VANTIV_PAYMENTS_ORDER_STATUS_COMPLETE_ID > 0) {
      $this->_order_status_complete = ADDONS_PAYMENT_VANTIV_PAYMENTS_ORDER_STATUS_COMPLETE_ID;
    }    

    if (is_object($order)) $this->update_status();
    
    if (defined('ADDONS_PAYMENT_VANTIV_PAYMENTS_TEST_MODE') && ADDONS_PAYMENT_VANTIV_PAYMENTS_TEST_MODE == '1') {
  ...
  */  
  public function update_status() {
    ...
    if ( ($this->_status === true) && ((int)ADDONS_PAYMENT_VANTIV_PAYMENTS_ZONE > 0) ) {
      $check_flag = false;
      ...
      $Qcheck->bindInt(':geo_zone_id', ADDONS_PAYMENT_VANTIV_PAYMENTS_ZONE);
  ...
  */   
  public function selection() {
    ...
    $selection = array('id' => $this->_code,
                       'module' => '<div class="payment-selection">' . $this->_method_title . '<span>' . $this->_card_images . '</span></div><div class="payment-selection-title">' . $lC_Language->get('payment_vantiv_method_blurb') . '</div>');    
  ...
  */ 
  public function check() {
    if (!isset($this->_check)) {
      $this->_check = defined('ADDONS_PAYMENT_VANTIV_PAYMENTS_STATUS');
    }
  ...
  */ 
  private function _query_uid($uID) {
    ...
    $uid_query_params = array('CRESecureID' => ADDONS_PAYMENT_VANTIV_PAYMENTS_LOGIN, 
                              'CRESecureAPIToken' => ADDONS_PAYMENT_VANTIV_PAYMENTS_API_TOKEN,
                              'uID' => $uID);    
  ...
  */ 
  private function _iframe_params($order_id) {
    ...
    $uid_action_params = array('CRESecureID' => ADDONS_PAYMENT_VANTIV_PAYMENTS_LOGIN, 
                               'CRESecureAPIToken' => ADDONS_PAYMENT_VANTIV_PAYMENTS_API_TOKEN,
                               ...
                               'content_template_url' => (getenv('HTTPS') == 'on') ? lc_href_link('addons/Vantiv_Payments/vantiv_template.php', '', 'SSL', true, true, true) : null,
