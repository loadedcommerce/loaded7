...
try {
    ...
    if(ADDONS_PAYMENT_PAYPAL_PAYMENTS_STANDARD_IPN_DEBUG == 'Yes') {
...
if ($verified) {
  ...
  if (defined('ADDONS_PAYMENT_PAYPAL_PAYMENTS_STANDARD_IPN_DEBUG') && ADDONS_PAYMENT_PAYPAL_PAYMENTS_STANDARD_IPN_DEBUG == 'Yes') {
    @mail(ADDONS_PAYMENT_PAYPAL_PAYMENTS_STANDARD_IPN_DEBUG_EMAIL, 'Verified IPN', $listener->getTextReport());
  }
} else {
  ...
  lC_Order::process($ipn_order_id, ADDONS_PAYMENT_PAYPAL_PAYMENTS_STANDARD_ORDER_CANCELED_STATUS_ID);
  $response_array['root']['transaction_response'] = 'INVALID';
  $ipn_transaction_response = 'INVALID';
  if (defined('ADDONS_PAYMENT_PAYPAL_PAYMENTS_STANDARD_IPN_DEBUG') && ADDONS_PAYMENT_PAYPAL_PAYMENTS_STANDARD_IPN_DEBUG == 'Yes') {
    @mail(ADDONS_PAYMENT_PAYPAL_PAYMENTS_STANDARD_IPN_DEBUG_EMAIL, 'Invalid IPN', $listener->getTextReport());
  }  
}
