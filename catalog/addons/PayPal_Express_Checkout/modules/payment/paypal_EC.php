<?php
/**  
  $Id: paypal_EC.php v1.0 2013-01-01 gulsarrays $
...
class lC_Payment_paypal_EC extends lC_Payment {     
 /**
  ...
  * Constructor
  */      
  public function lC_Payment_paypal_EC() {
    global $lC_Language;

    $this->_title = $lC_Language->get('payment_paypal_EC_title'); // admin listing title
    $this->_method_title = $lC_Language->get('payment_paypal_EC_method_title'); // public sidebar title 
    $this->_status = (defined('ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_STATUS') && (ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_STATUS == '1') ? true : false);
    $this->_sort_order = (defined('ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_SORT_ORDER') ? ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_SORT_ORDER : null);

    if (defined('ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_STATUS')) {
      $this->initialize();
    }
  }
 /**
  ...
  */
  public function initialize() {
    ...
    if ((int)ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_ORDER_STATUS_ID > 0) {
      $this->order_status = ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_ORDER_STATUS_ID;
    }
    
    if ((int)ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_ORDER_STATUS_COMPLETE_ID > 0) {
      $this->_order_status_complete = ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_ORDER_STATUS_COMPLETE_ID;
    }    

    if (is_object($order)) $this->update_status();
    
    if (defined('ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_TEST_MODE') && ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_TEST_MODE == '1') {
  ...
  */  
  public function update_status() {
    ...
    if ( ($this->_status === true) && ((int)ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_ZONE > 0) ) {
      $check_flag = false;
      ...
      $Qcheck->bindInt(':geo_zone_id', ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_ZONE);
  ...
  */   
  public function selection() {
    ...
    $selection = array('id' => $this->_code,
                       'module' => '<div class="payment-selection">' . $lC_Language->get('payment_paypal_EC_method_title') . '</div><div class="payment-selection-title">' . $lC_Language->get('payment_paypal_EC_method_blurb') . '</div>');    
    ...
    switch ($result) {
      ...
      default :
        if ($result == NULL) { // customer clicked cancel
          ...
          $errmsg = sprintf($lC_Language->get('payment_paypal_EC_error_occurred'), '(' . $dataArr['L_ERRORCODE0'] . ') ' . $dataArr['L_SHORTMESSAGE0']);
          $error = true;
        }
    }   
  ...
  */ 
  public function check() {
    if (!isset($this->_check)) {
      $this->_check = defined('ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_STATUS');
    }
  ...
  * @return string
  */
  private function _getUserParams($method) {
    return "METHOD=" . $method .
           "&VERSION=98.0" . 
           "&USER=" . urlencode(ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_API_USERNAME) .
           "&PWD=" . urlencode(ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_API_PASSWORD). 
           "&SIGNATURE=" . urlencode(ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_API_SIGNATURE);
  }
 /**
  ...
  */  
  private function _ec_process() {
    ...
    if ($lC_Customer->isLoggedOn() === false) {
      ...
      $lC_ShoppingCart->setBillingMethod(array('id' => 'paypal_EC', 'title' => $GLOBALS['lC_Payment_paypal_EC']->getMethodTitle()));
  ...
  */  
  private function _doExpressCheckoutPayment($token, $payerID) {
    ...
    $returnUrl = (defined('ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_TEMPLATE') && ADDONS_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_TEMPLATE == 'IFRAME') ?  lc_href_link(FILENAME_IREDIRECT, '', 'SSL', true, true, true) : lc_href_link(FILENAME_CHECKOUT, 'process', 'SSL', true, true, true);
    ...
    if ($dataArr['ACK'] == 'Failure') { // other error  
      $lC_MessageStack->add('shopping_cart', sprintf($lC_Language->get('payment_paypal_EC_error_occurred'), '(' . $dataArr['L_ERRORCODE0'] . ') ' . $dataArr['L_SHORTMESSAGE0']), 'error');
      return false;
    }  
