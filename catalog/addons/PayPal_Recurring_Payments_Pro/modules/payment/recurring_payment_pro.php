<?php
/**  
  $Id: recurring_payment_pro.php v1.0 2013-01-01 gulsarrays $
...
class lC_Payment_recurring_payment_pro extends lC_Payment {     
 /**
  ...
  * @access protected
  */  
  protected $_code = 'recurring_payment_pro';
 /**
  ...
  * Constructor
  */      
  public function lC_Payment_recurring_payment_pro() {
    global $lC_Language;

    $this->_title = $lC_Language->get('payment_paypal_recurring_pro_title'); // admin listing title
    $this->_method_title = $lC_Language->get('payment_paypal_recurring_pro_method_title'); // public sidebar title 
    $this->_status = (defined('ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_STATUS') && (ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_STATUS == '1') ? true : false);
    $this->_sort_order = (defined('ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_SORT_ORDER') ? ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_SORT_ORDER : null);

    if (defined('ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_STATUS')) {
      $this->initialize();
    }
  }
 /**
  ...
  */
  public function initialize() {
    ...
    if ((int)ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_ORDER_STATUS_ID > 0) {
      $this->order_status = ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_ORDER_STATUS_ID;
    }
    
    if ((int)ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_ORDER_STATUS_COMPLETE_ID > 0) {
      $this->_order_status_complete = ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_ORDER_STATUS_COMPLETE_ID;
    }    

    if (is_object($order)) $this->update_status();    
    if (defined('ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_TEST_MODE') && ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_TEST_MODE == '1') {
  ...
  */  
  public function update_status() {
    ...
    if ( ($this->_status === true) && ((int)ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_ZONE > 0) ) {
      $check_flag = false;
      ...
      $Qcheck->bindInt(':geo_zone_id', ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_ZONE);
  ...
  */   
  public function selection() {
    ...
    if((int)ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_DP_STATUS) {
      
      $card_accepted_types = ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_ACCEPTED_TYPES;
      ...
      $selection = array('id' => $this->_code,
                         'module' => '<div class="payment-selection">' . $lC_Language->get('payment_paypal_recurring_pro_method_title') . '</div><div class="payment-selection-title">' . $lC_Language->get('payment_paypal_recurring_pro_method_blurb') . '</div>',
                         'fields' => array(
                                        array('title' => $lC_Language->get('payment_paypal_recurring_pro_cc_firstname'),
                                              'field' => lc_draw_input_field('recurring_payment_pro_cc_firstname', $lC_ShoppingCart->getBillingAddress('firstname'), ' class="form-control"')),
                                        array('title' => $lC_Language->get('payment_paypal_recurring_pro_cc_lastname'),
                                              'field' => lc_draw_input_field('recurring_payment_pro_cc_lastname', $lC_ShoppingCart->getBillingAddress('lastname'), ' class="form-control"')),
                                        array('title' => $lC_Language->get('payment_paypal_recurring_pro_cc_type'),
                                              'field' => lc_draw_pull_down_menu('recurring_payment_pro_cc_type', $cc_array, ' class="form-control"')),
                                        array('title' => $lC_Language->get('payment_paypal_recurring_pro_cc_number'),
                                              'field' => lc_draw_input_field('recurring_payment_pro_cc_number', NULL, ' class="form-control"')),
                                        array('title' => $lC_Language->get('payment_paypal_recurring_pro_cc_expiry'),
                                              'field' => lc_draw_pull_down_menu('recurring_payment_pro_cc_expires_month', $expires_month, ' class="form-control"') . '&nbsp;' . lc_draw_pull_down_menu('recurring_payment_pro_cc_expires_year', $expires_year, ' class="form-control"')),
                                        array('title' => $lC_Language->get('payment_paypal_recurring_pro_cc_cvv').' '.$lC_Language->get('payment_paypal_recurring_pro_cc_cvv_text'),
                                              'field' => lc_draw_input_field('recurring_payment_pro_cc_cvv', NULL, ' class="form-control" SIZE="4" MAXLENGTH="4"'))
                                     )
                        );
    }
    return $selection;
  ...
  */ 
  public function pre_confirmation_check() {
    ...
    $lC_ShoppingCart->setBillingMethod(array('id' => 'recurring_payment_pro', 'title' => ((isset($_SESSION['recurring_payment_pro_ec']) && $_SESSION['recurring_payment_pro_ec'] == 1) ? $lC_Language->get('payment_paypal_recurring_pro_express_method_title') : $this->_method_title)));
    return false;
  }
 /**
  ...
  */ 
  public function process_button() { 
    ...
    if($_SESSION['recurring_payment_pro_ec'] == 0) { // Direct Credit Card Payment
      if(!empty($_POST['recurring_payment_pro_cc_firstname']) ===  false || !empty($_POST['recurring_payment_pro_cc_lastname']) ===  false || !empty($_POST['recurring_payment_pro_cc_number']) ===  false || !empty($_POST['recurring_payment_pro_cc_cvv']) ===  false ) {
        ...
        echo lc_draw_hidden_field('recurring_payment_pro_cc_type', $recurring_payment_pro_cc_type);
        echo lc_draw_hidden_field('recurring_payment_pro_cc_number', $recurring_payment_pro_cc_number);
        echo lc_draw_hidden_field('recurring_payment_pro_cc_expiry', $recurring_payment_pro_cc_expiry);
        echo lc_draw_hidden_field('recurring_payment_pro_cc_cvv', $recurring_payment_pro_cc_cvv);
      }
  ...
  */ 
  public function process() {
    ...
    if(!$_SESSION['recurring_payment_pro_ec']) { 
    ...
    switch ($result) {
      ...
      default :
        if ($result == NULL) { // customer clicked cancel
          ...
          $errmsg = sprintf($lC_Language->get('payment_paypal_recurring_pro_error_occurred'), '(' . $dataArr['L_ERRORCODE0'] . ') ' . $dataArr['L_SHORTMESSAGE0']);
          $error = true;
        }
    }   
    ...
    if (isset($_SESSION['recurring_payment_pro_ec'] )) unset($_SESSION['recurring_payment_pro_ec']);    
  ...
  */ 
  public function check() {
    if (!isset($this->_check)) {
      $this->_check = defined('ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_STATUS');
    }
    return $this->_check;
  }
 /**
  ...
  */   
  public function createRecurringPaymentsProfile() {
    ...
    $response = $this->_createRecurringPaymentsProfile($_SESSION['PPEC_PAYDATA']['TOKEN'], $_SESSION['PPEC_PAYDATA']['PAYERID']);
  ...
  */
  private function _getUserParams($method,$version='98.0') {
    return "METHOD=" . $method .  
           "&VERSION=" . $version .
           "&USER=" . urlencode(ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_API_USERNAME) .
           "&PWD=" . urlencode(ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_API_PASSWORD). 
           "&SIGNATURE=" . urlencode(ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_API_SIGNATURE);
  }
 /**
  ...
  */  
  private function _ec_process() {
    ...
    if ($lC_Customer->isLoggedOn() === false) {
      ...
      $lC_ShoppingCart->setBillingMethod(array('id' => 'recurring_payment_pro', 'title' => $GLOBALS['lC_Payment_recurring_payment_pro']->getMethodTitle()));
  ...
  * @return string
  */  
  private function _createRecurringPaymentsProfile($token, $payerID) {
    ...
    $transType = ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_TRXTYPE;
    ...
    if( $billingPeriod == 'SemiMonth' ) {
      $billingFrequency = 1;
    }
    $returnUrl = (defined('ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_TEMPLATE') && ADDONS_PAYMENT_PAYPAL_RECURRING_PAYMENTS_PRO_TEMPLATE == 'IFRAME') ?  lc_href_link(FILENAME_IREDIRECT, '', 'SSL', true, true, true) : lc_href_link(FILENAME_CHECKOUT, 'process', 'SSL', true, true, true);

    $postData = $this->_getUserParams('CreateRecurringPaymentsProfile') .  
                "&PAYMENTREQUEST_0_PAYMENTACTION=" . $transType . 
                "&PROFILESTARTDATE=" . date("Y-m-d h:m:i") .
                "&BILLINGPERIOD=" . $billingPeriod .
                "&BILLINGFREQUENCY=" . $billingFrequency .
                "&DESC=" . urlencode('Loaded7 Subscription') .                     
    ...
    if ($dataArr['ACK'] == 'Failure') { // other error  
      $lC_MessageStack->add('shopping_cart', sprintf($lC_Language->get('payment_paypal_recurring_pro_error_occurred'), '(' . $dataArr['L_ERRORCODE0'] . ') ' . $dataArr['L_SHORTMESSAGE0']), 'error');
      return false;
    }  
  ...
  */  
  private function _DoDirectPayment() {
    ...
    $postData = $this->_getUserParams('CreateRecurringPaymentsProfile') .
                ...
                "&INITAMT=" . $lC_Currencies->formatRaw($lC_ShoppingCart->getTotal(), $lC_Currencies->getCode()) . 
                "&AMT=" . $lC_Currencies->formatRaw($lC_ShoppingCart->getTotal(), $lC_Currencies->getCode()) . 
                "&ACCT=" . $recurring_payment_pro_cc_number .
                "&CREDITCARDTYPE=" . $recurring_payment_pro_cc_type .
                "&CVV2=" . $recurring_payment_pro_cc_cvv .
                "&EXPDATE=" . $recurring_payment_pro_cc_expiry .                
                "&IPADDRESS=" . lc_get_ip_address() .                 
                "&FIRSTNAME=" . urlencode($recurring_payment_pro_cc_firstname) . 
                "&LASTNAME=" . urlencode($recurring_payment_pro_cc_lastname) . 
