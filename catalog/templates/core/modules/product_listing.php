...
if ($Qlisting->numberOfRows() > 0) {
  ...
  while ($Qlisting->next()) {
    ...
    $limited = $lC_Product->isLimited();
    $qty = $lC_Product->getQuantity();
    if($limited && $qty > 0){
      $show = true;
    }elseif($limited && $qty < 1){
      $show = false;
    }else{
      $show = true;
    }
    if ( $show && strtotime($lC_Product->getDateAvailable()) <= strtotime(lC_Datetime::getShort()) ) {
      ...
      for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
        switch ($column_list[$col]) {
          ...
          case 'PRODUCT_LIST_PRICE':
            if( $lC_Product->isDiscontinued() ) {
              $image = '<img src="'.DIR_WS_TEMPLATE_IMAGES.'/not.png" />';
            }elseif( $lC_Product->isSpecialOrder() ) {
              $image = '<img src="'.DIR_WS_TEMPLATE_IMAGES.'/special-order.png" />';
            }else{
              $image = '';
            }
            $output .= '<div class="product-listing-module-price pricing-row">' . (($show_price) ? $lC_Product->getPriceFormated(true) : null) . $image . '</div>' . "\n";
            break;
          ...
          case 'PRODUCT_LIST_BUY_NOW':
            if($lC_Product->getMinPurchase() > 1){
              ...
              if($lC_Product->getPacks() > 1){
                $min_line = sprintf($lC_Language->get('text_qty_min_list'), $lC_Product->getMinPurchase(), $lC_Product->getPacks());
              }else{
                $min_line = sprintf($lC_Language->get('text_qty_min_list_min'), $lC_Product->getMinPurchase());
              }
            } 

            if ($lC_Product->isDiscontinued() || $lC_Product->isSpecialOrder() ) {
              $output .= '<div class="product-listing-module-buy-now buy-btn-div">' . lc_link_object(lc_href_link(FILENAME_PRODUCTS, $lC_Product->getKeyword()), '<div class="product-listing-module-buy-now-button">'.$lC_Language->get('more_information').'</div>') . '</div>';
            }else{
