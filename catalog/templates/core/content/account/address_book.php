      ...
      <div class="col-sm-12 col-lg-12">
        ...
        while ($Qaddresses->next()) {
          ...
          <div class="btn-group clearfix absolute-top-right-large-padding">
            <form action="<?php echo lc_href_link(FILENAME_ACCOUNT, 'address_book=' . $Qaddresses->valueInt('address_book_id') . '&edit', 'AUTO'); ?>" class="display-inline" method="post"><button onclick="$(this).closest('form').submit();" type="button" class="btn btn-default btn-xs"><?php echo $lC_Language->get('button_edit'); ?></button></form>
            <form action="<?php echo lc_href_link(FILENAME_ACCOUNT, 'address_book=' . $Qaddresses->valueInt('address_book_id') . '&delete', 'AUTO'); ?>" class="display-inline" method="post"><button onclick="$(this).closest('form').submit();" type="button" class="btn btn-default btn-xs"><?php echo $lC_Language->get('button_delete'); ?></button></form>
          </div>
          <?php
          echo '</div>' . "\n";
        }
      ...
      <div class="col-sm-12 col-lg-12">
        <div class="btn-set small-margin-top clearfix">
          ...
          if ($Qaddresses->numberOfRows() < MAX_ADDRESS_BOOK_ENTRIES) {
            echo '<form action="' . lc_href_link(FILENAME_ACCOUNT, 'address_book&new', 'AUTO') . '" method="post"><button onclick="$(this).closest(\'form\').submit();" class="pull-right btn btn-lg btn-primary" type="submit">' . $lC_Language->get('button_add_address') . '</button></form>' . "\n";
          } else {
