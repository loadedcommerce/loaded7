  ...
  $(".content-specials-listing-container").each(function(){
    ...
    descContent = descContent.replace(descContentText, newDescContentText);      
     ...
     output = '<div class="' + thisContentClass+ ' with-padding">'+
             ...
             '      </div>' +
             '    </div>' +
             '    <div class="az_wishlistpart thumbButtons"> ' + 
             '      <div class="info display-inline-block col-sm-4">'+ nameContent +'</div> '+         
             '      <div class="wishlist display-inline-block col-sm-4"><a href="javascript:void(0);" >' + textAddToWishlist + '</a></div> '+    
             '       <div class="cart display-inline-block col-sm-4">'+ buttonContent +'</div> ' +
             '      </div> '+
             '  </div>' +
             '</div>';     
