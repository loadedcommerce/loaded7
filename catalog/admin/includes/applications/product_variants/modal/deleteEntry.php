    ...
    function (data) {
      ...
      $.modal({
        content: '<div id="deleteZone">'+
                 ...
                 '    <p id="deleteConfirmMessage"><?php echo $lC_Language->get('introduction_delete_entry'); ?>'+
                 '      <p><b>' + decodeURI(name.replace(/\+/g, '%20')) + '</b></p>'+
                 '    </p>'+
                 '  </div>'+
                 '</div>',
        title: '<?php echo $lC_Language->get('modal_heading_delete_entry'); ?>',
        width: 300,
            actions: {
          'Close' : {
            color: 'red',
        ...
        buttons: {
          ...
          '<?php echo $lC_Language->get('button_delete'); ?>': {
            ...
            click:    function(win) {
              var jsonLink = '<?php echo lc_href_link_admin('rpc.php', $lC_Template->getModule() . '=' . $_GET[$lC_Template->getModule()] . '&action=deleteEntry&pveid=PVEID'); ?>';
              $.getJSON(jsonLink.replace('PVEID', parseInt(id)),
                function (data) {
                  if (data.rpcStatus == -10) { // no session
                    var url = "<?php echo lc_href_link_admin(FILENAME_DEFAULT, 'login'); ?>";
