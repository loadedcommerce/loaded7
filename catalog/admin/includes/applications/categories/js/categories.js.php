    ...
    if (ENABLE_EDITOR == 1 && EDITOR_CONFIGURATION_CATEGORY != 'Off') { 
      if (USE_DEFAULT_TEMPLATE_STYLESHEET == 1) {
        foreach ( $lC_Language->getAll() as $l ) {  
          echo "CKEDITOR.replace('ckEditorCategoriesDescription_" . $l['id'] . "', { toolbar: '" . ((EDITOR_CONFIGURATION_CATEGORY == 'Full') ? strtoupper(EDITOR_CONFIGURATION_CATEGORY) : EDITOR_CONFIGURATION_CATEGORY) . "', height: 200, width: '99%', filebrowserUploadUrl: '../ext/jquery/ckeditor/ck_upload.php', contentsCss: '../templates/" . DEFAULT_TEMPLATE . "/css/styles.css', stylesSet: [] });";
        }
      } else {
        foreach ( $lC_Language->getAll() as $l ) {  
          echo "CKEDITOR.replace('ckEditorCategoriesDescription_" . $l['id'] . "', { toolbar: '" . ((EDITOR_CONFIGURATION_CATEGORY == 'Full') ? strtoupper(EDITOR_CONFIGURATION_CATEGORY) : EDITOR_CONFIGURATION_CATEGORY) . "', height: 200, width: '99%', filebrowserUploadUrl: '../ext/jquery/ckeditor/ck_upload.php' });";
        }
      }
    } else {
      foreach ( $lC_Language->getAll() as $l ) {  
