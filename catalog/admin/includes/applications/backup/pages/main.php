...
*/
$lastRestoreDate = ( defined('DB_LAST_RESTORE') ) ? '<table><tr><td>' . $lC_Language->get('last_restoration_date') . ' ' . DB_LAST_RESTORE . '</td><td><a href="javascript(void);" onclick="doForget();"><img border="0" src="' .  lc_icon_admin_raw('delete.png') . '" title="' . $lC_Language->get('forget_restoration_date') . '"></a></td></tr></table>' : '';
?>
<!-- Main content -->
        ...
        <tr>
          ...
          <th scope="col" class="align-right">
            <span class="button-group compact" style="white-space:nowrap;">
              <a style="display:none;" href="javascript(void);" style="cursor:pointer" class="on-mobile button with-tooltip icon-outbox green<?php echo (((int)$_SESSION['admin']['access'][$lC_Template->getModule()] < 2) ? ' disabled' : NULL); ?>" href="<?php echo (((int)$_SESSION['admin']['access'][$lC_Template->getModule()] < 2) ? '#' : 'javascript://" onclick="doBackup(); return false;'); ?>" title="<?php echo $lC_Language->get('button_backup'); ?>"></a>
              <a style="display:none;" href="javascript(void);" style="cursor:pointer" class="on-mobile button with-tooltip icon-inbox blue<?php echo (((int)$_SESSION['admin']['access'][$lC_Template->getModule()] < 3) ? ' disabled' : NULL); ?>" href="<?php echo (((int)$_SESSION['admin']['access'][$lC_Template->getModule()] < 3) ? '#' : 'javascript://" onclick="restoreLocal(); return false;'); ?>" title="<?php echo $lC_Language->get('button_restore'); ?>"></a>
