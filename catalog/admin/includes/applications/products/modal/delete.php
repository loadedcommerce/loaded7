...
function deleteProduct(id, name) {
  ...
  $.getJSON(jsonLink.replace('PID', parseInt(id)),
    function (data) {
      ...
      $.modal({
        content: '<div id="deleteProduct">'+
                 '  <div id="deleteConfirm">'+
                 '    <p id="deleteConfirmMessage"><?php echo $lC_Language->get('introduction_delete_product'); ?>'+
                 '      <p><b>' + decodeURI(name.replace(/\+/g, '%20')) + '</b></p>'+
                 '    </p>'+
                 '  </div>'+
                 '</div>',
        title: '<?php echo $lC_Language->get('modal_heading_delete_product'); ?>',
        width: 300,
        scrolling: false,
        actions: {
          'Close' : {
        ...
        buttons: {
          ...
          '<?php echo $lC_Language->get('button_delete'); ?>': {
            ...
            click:    function(win) {
              var jsonLink = '<?php echo lc_href_link_admin('rpc.php', $lC_Template->getModule() . '&action=deleteProduct&pid=PID'); ?>';
              $.getJSON(jsonLink.replace('PID', parseInt(id)),
                function (data) {
                  if (data.rpcStatus == -10) { // no session
                    var url = "<?php echo lc_href_link_admin(FILENAME_DEFAULT, 'login'); ?>";
