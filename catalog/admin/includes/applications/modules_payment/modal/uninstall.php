...
function uninstallModule(id, name) {
  ...
  $.modal({
    content: '<div id="uninstallModule">'+
             ...
             '    <p id="uninstallModuleConfirmMessage"><?php echo $lC_Language->get('introduction_uninstall_payment_module'); ?>'+
             '      <p><b>' + decodeURI(name.replace(/\+/g, '%20')) + '</b></p>'+
             '    </p>'+
             '  </div>'+
             '</div>',
    title: '<?php echo $lC_Language->get('modal_heading_uninstall_payment_module'); ?>',
    width: 300,
    scrolling: false,
    actions: {
      'Close' : {
