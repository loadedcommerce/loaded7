...
function batchDelete() {
  ...
  $.modal({
    content: '<div id="batchDelete">'+
             ...
             '    <p id="batchDeleteConfirmMessage"><?php echo $lC_Language->get('introduction_batch_delete_countries'); ?>'+
          ...
          $.getJSON(jsonLink.replace('BATCH', values),
            function (data) {
              ...
              if (data.namesString.length > 0) {
                $.modal.alert('<?php echo $lC_Language->get('batch_delete_warning_country_in_use'); ?> ' + "\n\n" + data.namesString);
                oTable.fnDraw();
                return false;
              }
              if (data.rpcStatus != 1) {
