...
function batchDelete() {
  ...
  $.modal({
    content: '<div id="batchDelete">'+
             ...
             '    <p id="batchDeleteConfirmMessage"><?php echo $lC_Language->get('introduction_batch_delete_reviews'); ?>'+
             '      <p><b>' + decodeURI(name.replace(/\+/g, '%20')) + '</b></p>'+
             '    </p>'+
             '  </div>'+
             '</div>',
    title: '<?php echo $lC_Language->get('modal_heading_batch_delete_reviews'); ?>',
    width: 300,
    scrolling: false,
    actions: {
      'Close' : {
