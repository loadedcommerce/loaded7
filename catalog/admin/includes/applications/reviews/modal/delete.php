...
function deleteEntry(id, name) {
  ...
  $.modal({
    content: '<div id="deleteEntry">'+
             ...
             '    <p id="deleteConfirmMessage"><?php echo $lC_Language->get('introduction_delete_review'); ?>'+
             '      <p><b>' + decodeURI(name.replace(/\+/g, '%20')) + '</b></p>'+
             '    </p>'+
             '  </div>'+
             '</div>',
    title: '<?php echo $lC_Language->get('modal_heading_delete_review'); ?>',
    width: 300,
    scrolling: false,
    actions: {
      'Close' : {
    ...
    buttons: {
      ...
      '<?php echo $lC_Language->get('button_delete'); ?>': {
        ...
        click:    function(win) {
          var jsonLink = '<?php echo lc_href_link_admin('rpc.php', $lC_Template->getModule() . '&action=deleteEntry&rid=RID'); ?>';
