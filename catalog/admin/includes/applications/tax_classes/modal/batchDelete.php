...
function batchDeleteClasses() {
  ...
  $.modal({
    content: '<div id="batchDeleteClasses">'+
             '  <div id="batchDeleteClassesConfirm">'+
             '    <p id="batchDeleteClassesConfirmMessage"><?php echo $lC_Language->get('introduction_batch_delete_tax_classes'); ?>'+
    ...
    buttons: {
      ...
      '<?php echo $lC_Language->get('button_delete'); ?>': {
        ...
        click:    function(win) {
          var jsonLink = '<?php echo lc_href_link_admin('rpc.php', $lC_Template->getModule() . '&action=batchDeleteClasses&BATCH'); ?>';
          $.getJSON(jsonLink.replace('BATCH', values),
            function (data) {
              ...
              if (data.namesString.length > 0) {
                $.modal.alert('<?php echo $lC_Language->get('batch_delete_warning_tax_class_in_use'); ?> ' + data.namesString);
                return false;
              }
              if (data.rpcStatus != 1) {
