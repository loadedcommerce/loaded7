...
function batchDelete() {
  ...
  $.modal({
    content: '<div id="batchDelete">'+
             ...
             '    <p id="batchDeleteConfirmMessage"><?php echo $lC_Language->get('introduction_batch_delete_zone_groups'); ?>'+
          ...
          $.getJSON(jsonLink.replace('BATCH', values),
            function (data) {
              ...
              if (data.namesString.length > 0) {
                $.modal.alert('<?php echo $lC_Language->get('batch_delete_warning_group_in_use_tax_rate'); ?> ' + data.namesString);
                oTable.fnReloadAjax();
                return false;
              }
              if (data.rpcStatus != 1) {
